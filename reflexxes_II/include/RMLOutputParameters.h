...
//  ----------------------------------------------------------
    RMLOutputParameters(const unsigned int DegreesOfFreedom)
    {
		...
		this->ProfileVector							=	new RMLIntVector(DegreesOfFreedom)		;
		...
		memset(this->ProfileVector->VecData						,	0x0	,		DegreesOfFreedom * sizeof(int))		;		
...
//  ----------------------------------------------------------
    RMLOutputParameters(const RMLOutputParameters &OP)
    {
		...
		this->ProfileVector							=	new RMLIntVector(this->NumberOfDOFs)					;
...
//  ----------------------------------------------------------	
    RMLOutputParameters &operator = (const RMLOutputParameters &OP)
    {
		...
		OP.GetProfileVector			(this->ProfileVector						)		;
...
//  ----------------------------------------------------------
    ...
    inline void GetProfileVector(RMLIntVector *OutputVector) const
    {
        *OutputVector	=	*(this->ProfileVector);
    }

    inline void GetProfileVector(		int					*OutputVector
                                    ,	const unsigned int	&SizeInBytes) const
    {
        memcpy(		(void*)OutputVector
                ,	(void*)this->ProfileVector->VecData
                ,	SizeInBytes	);
    }

    inline void GetProfileVectorElement(		int				*OutputValue
                                            ,	const unsigned int	&Index) const
    {
        if ( ( Index + 1 ) > ((unsigned int) this->ProfileVector->GetVecDim() ) )
        {
            *OutputValue	=	0;
        }
        else
        {	
            *OutputValue	=	(*this->ProfileVector)[Index];
        }
    }

    inline int GetProfileVectorElement(const unsigned int &Index) const
    {
        if ( ( Index + 1 ) > ((unsigned int) this->ProfileVector->GetVecDim() ) )
        {
            return(0);
        ...
        {		
            return( (*this->ProfileVector)[Index] );
        }		
    }
